...
def calculate_score_per_song(model, dataloader, device, ref_midi_notes, est_midi_notes):
    """
    ...
    if 'onset_output' in outputs.keys() and 'offset_output' in outputs.keys():
        ref_on_off_pairs = np.array(list(zip(targets['onset_roll'].flatten(), targets['onset_roll'].flatten())))
        est_on_offs = np.array(list(zip(outputs['onset_roll'].flatten(), targets['outputs'].flatten())))
        ...
        note_precision, note_recall, note_f1, _ = \
                    mir_eval.transcription.precision_recall_f1_overlap(
                        ...
                        est_pitches=librosa.note_to_hz(est_midi_notes), 
    ...
    if 'reg_onset_output' and 'reg_offset_output' in outputs.keys():
        ref_on_off_pairs = np.array(list(zip(targets['reg_onset_output'].flatten(), targets['reg_onset_roll'].flatten())))
        est_on_offs = np.array(list(zip(outputs['reg_offset_output'].flatten(), targets['reg_onset_roll'].flatten())))
