...
from noteify.core.config import (SAMPLE_RATE, FMIN_FREQ, BINS_PER_NOTE, BINS_PER_OCTAVE, NUM_NOTES, NUM_BINS,
...
class ResNetFreqConvBlock(nn.Module):
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(in_channels=in_channels, 
                               out_channels=out_channels,
                               kernel_size=kernel1_size, stride=(1, 1),
                               bias=False)
                              
        self.conv2 = nn.Conv2d(in_channels=out_channels, 
                               out_channels=out_channels,
                               kernel_size=kernel2_size, stride=(1, 1),
                               bias=False)
...
class AcousticCRNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv_block2 = ResNetFreqConvBlock(in_channels=48, out_channels=64, kernel1_size=(SEGMENT_FRAMES, 1), kernel2_size=(1, NUM_BINS//2))
    ...
    def forward(self, x):
        ...
        x =  x.transpose(1, 2).flatten(2) # (batch_size, time_steps, channels_num * freq_bins4)
...
class TranscriptionNN(nn.Module):
    ...
    def forward(self, x, apply_sigmoid=False):
        ...
        x = self.spec_layer() # (batch_size, num_frames, freq_bins)
        x = x.unsqueeze(1) # (batch_size, num_channels, num_frames, freq_bins)
        x = x.transpose(1, 3)
        x = self.bn(x) # normalize spectrogram
        ...
        frame_output, _ = self.frame_layer(cond_input)
...
if __name__ == '__main__':
    ...
    x = torch.zeros((1, SEGMENT_SAMPLES))
    x = model(x)
